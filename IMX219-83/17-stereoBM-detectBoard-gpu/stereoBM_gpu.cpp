...
using namespace cv;
using std::cout; using std::cerr; using std::endl; using std::runtime_error; using std::vector;
...
int main(int argc, char** argv)
{
    ...
    CommandLineParser parser(argc, argv, "{@arg1||}{@arg2||}{w||}{h||}{ndisp|0|}{i||}{e||}{scale|1|}{blocksize|0|}");
    ...
    {
        case CHESSBOARD:
            found = findChessboardCorners( left, boardSize, pointbuf,
                CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_FAST_CHECK | CALIB_CB_NORMALIZE_IMAGE);
            break;
        case CIRCLES_GRID:
            found = findCirclesGrid( left, boardSize, pointbuf );
            break;
        case ASYMMETRIC_CIRCLES_GRID:
            found = findCirclesGrid( left, boardSize, pointbuf, CALIB_CB_ASYMMETRIC_GRID );
            break;
        default:
    ...
    if(found){
        ...
        for (Point2f point : pointbuf){
            ushort temp = depth.at<ushort>(point.y, point.x);
            if (temp >= 65535)
                continue;
            sumDist += temp;
            pointNum++;
            circle(left, CvPoint(point.x, point.y),5,cvScalar(255));
            // cout << temp << endl;
        }
        // cout << sumDist << " " << pointbuf.size() << endl;
        if (pointNum > 0){
            dist = (double) sumDist / (double) pointNum;
            cout << "Average distance to the Board from the camera is " << dist << " (mm)" << endl;
        }
    }
