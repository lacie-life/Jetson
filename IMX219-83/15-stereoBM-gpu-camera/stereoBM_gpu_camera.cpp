...
{    
    ...
    while(1)
    {
        ...
        if (frame0.empty()) throw runtime_error("can't open file \"" + img1_filename + "\"");
        if (frame1.empty()) throw runtime_error("can't open file \"" + img2_filename + "\"");
        ...
        Mat disp(left.size(), CV_8U), disp8(left.size(), CV_8U);
        ...
        disp.convertTo(disp8, CV_8U, 255/(ndisp));
